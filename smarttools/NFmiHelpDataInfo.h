...
class NFmiHelpDataInfo
{
 public:
  ...
  // t‰m‰ on viritys, ett‰ olisi funktio, jolla voidaan pyyt‰‰ k‰ytetty fileFilter, riippuen siit‰
  // onko cache k‰ytˆss‰ vai ei
 ...
 private:
  ...
  std::string itsPartialDataCacheFileNameFilter;  // cachen k‰yttˆ ja partial-datat ovat oma lukunsa
                                                  // ja niit‰ varten pit‰‰ tehd‰ filefiltteri
  bool fForceFileFilterName;      // jos t‰m‰ on erikoistilanteissa asetettu true:ksi, ei v‰litet‰
                                  // mahdillisista cachen k‰ytˆist‰, vaan UsedFilefilter-metodi
                                  // palauttaa itsFileNameFilter:in arvon
  std::string itsLatestFileName;  // kun filtterill‰ on haettu tiedostot ja uusin on lˆytynyt,
                                  // talletetaan se t‰h‰n
  std::string itsLatestErroneousFileName;  // Jos datatiedosto on jotenkin korruptoitunut,
                                           // talletetaan viimeisimm‰n sellaisen nimi, jotta
                                           // lokituksen yhteydess‰ ei tarvitse minuutin v‰lein
  ...
  int itsFakeProducerId;  // joskus pit‰‰ muuttaa tuottaja id ennenkuin data otetaan k‰yttˆˆn esim.
                          // kepa data, jos t‰m‰ on 0, ei tehd‰ mit‰‰n

  // Seuraavat koskevat vain image-tyyppisi‰ juttuja (muissa n‰it‰ ei lueta/talleteta tiedostoon)
  std::string
      itsImageProjectionString;  // jos data on image-tyyppist‰ (satel,radar, jne.), on tiedostossa
  // stringi, joka sis‰lt‰‰ projektio tiedon jonka NFmiAreaFactory ymm‰rt‰‰
  NFmiDataIdent itsImageDataIdent;           // tieto image paramin id:st‰, nimest‰ (vain id ja nimi
                                             // talletetaan tiedostoon)
  boost::shared_ptr<NFmiArea> itsImageArea;  // t‰h‰n luodaan ed. stringin avulla projektio, t‰m‰ ei
                                             // ole tallessa tiedostossa
  // Edelliset koskevat vain image-tyyppisi‰ juttuja

  bool fNotifyOnLoad;  // Jos datan latauksen yhteydess‰ halutaan tehd‰ ilmoitus, t‰m‰ on true.
                       // Oletus arvo on false
  std::string itsNotificationLabel;  // Jos notifikaatioon halutaan tietty sanoma, se voidaan antaa
                                     // t‰h‰n. Defaulttina annetaan tiedoston nimi
  std::string itsCustomMenuFolder;   // Jos data halutaan laittaa haluttuun hakemistoon
                                     // param-popupeissa, tehd‰‰ sellainen asetus helpdata
  // konffeihin. Defaulttina tyhj‰, jolloin data menee 'normaaliin' paikkaansa valikoissa.
  int itsReportNewDataTimeStepInMinutes;  // Default arvo on 0, jolloin t‰ll‰ ei ole vaikutusta.
                                          // T‰m‰n avulla voidaan sanoa
  // ett‰ SmartMetin pit‰‰ tehd‰ raportointia 'puhekuplilla', kun tulee uutta dataa esim. uudelle
  // tunnille.
  std::string itsReportNewDataLabel;  // Jos halutaan tietty teksti viestiin, se lis‰t‰‰n t‰h‰n.
  std::string itsCombineDataPathAndFileName;  // jos t‰m‰ on m‰‰ritelty, tehd‰‰n fileNameFilterist‰
                                              // (hakemistosta) lˆytyvist‰ datoista yhdistelm‰
  // data ja se talletetaan t‰h‰n hakemistoon annetulla nimell‰ ja aikaleimalla (nimess‰ t‰hden
  // tilalle laitetaan aikaleima)
  int itsCombineDataMaxTimeSteps;  // jos ei haluaa rajoittaa kuinka iso yhdistelm‰ datasta tehd‰‰n,
                                   // t‰m‰n voi m‰‰ritell‰
  bool
      fMakeSoundingIndexData;  // jos t‰m‰ on true, SmartMet tekee datasta oman sounding-index datan
  std::string itsBaseNameSpace;
  int itsAdditionalArchiveFileCount;  // defaultti on 0, joitakin datoja (esim. kepa-datoja, joita
                                      // tuotetaan n. 15-20 per p‰iv‰)
  // halutaan s‰ilytt‰‰ enemm‰n kuin muita NFmiInfoOrganizer:issa. T‰ll‰ s‰‰dˆll‰ saadaan pidetty‰
  // ylim‰‰r‰iset datat.
  bool fEnable;  // T‰m‰ on pakollinen s‰‰tˆ optio siit‰, ett‰ onko data k‰ytˆss‰. T‰m‰ s‰‰t‰‰
                 // ladataanko dataa cacheen, SmartMEtiin, tehd‰‰nkˆ soundinIndex-dataa,
  // yhdistelm‰ datoja. Lis‰ksi tuleeko data popup-valikoihin, tai mihink‰‰n muihin datavalikoihin.
  bool fNonFixedTimeGab;  // T‰m‰ on valinnainen asetus. Onko datan malliajo v‰li m‰‰rittelem‰tˆn.
                          // Esim. silam-datat ovat ep‰m‰‰r‰isesti ajettuja,
  // myˆs virallinen editoitu data on m‰‰rittelem‰tˆn, mutta se hoidetaan erikois tapauksena.
  float itsModelRunTimeGapInHours;  // Joskus pit‰‰ laittaa tietyille datoille omat malliajo v‰lit,
                                    // jos ne poikkeavat totutuista
  // Esim. EC:ll‰ on normaalisti 12 h, mutta nyt 3vrk EC datat tulevat 6 tunnin v‰lein, joten 3 vrk
  // datoille pit‰‰ asettaa t‰m‰ 6:ksi.
...
class NFmiHelpDataInfoSystem
{
 ...
 private:
  ...
  checkedVector<NFmiHelpDataInfo> itsDynamicHelpDataInfos;  // t‰h‰n tulee jatkuvasti p‰ivitett‰v‰t
                                                            // datat kuten havainnot, tutka ja
                                                            // analyysi datat
  checkedVector<NFmiHelpDataInfo> itsStaticHelpDataInfos;   // t‰h‰n tulee kerran ladattavat jutut
  ...
  std::string itsCacheTmpDirectory;  // qdatat kopioidaan ensin t‰h‰n hakemistoon tmp-nimell‰ ja
                                     // lopuksi renametaan
                                     // oiekaan hakemistoon oikealla nimell‰
  // HUOM!! ett‰ tmp-file rename toimisi ns 'atomisesti', on hakemistojen oltava samalla
  // levypartitiolla!!!
  std::string itsCachePartialDataDirectory;  // combineData-threadin k‰ytt‰m‰t datat kopioidaan
                                             // t‰nne cacheen
  std::string
      itsCacheTmpFileNameFix;      // t‰m‰ name fix lis‰t‰‰n tmp tiedosto nimen alkuun ja loppuun
  ...
  float itsCacheFileKeepMaxDays;   // kuinka vanhat tiedostot ainakin siivotaan pois (esim. 1.5 on
                                   // 1.5 p‰iv‰‰ eli 36 tuntia) jos luku on <= 0 ei t‰t‰ k‰ytet‰
  int itsCacheMaxFilesPerPattern;  // kuinka monta tiedostoa maksimissaan pidet‰‰n kutakin tiedosto
                                   // patternia kohden, jos luku <= 0, ei t‰t‰ k‰ytet‰
