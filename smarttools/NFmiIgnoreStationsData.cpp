...
{
	std::string usedStationName = itsName; // aseman nimest‰ pit‰‰ poistaa ';' ja ':' merkit, lis‰ksi jos se on tyhj‰, laitetaan nimeksi "?"
...
{
	RemoveStation(unsigned long theId)
	:itsId(theId)
	{}
...
void NFmiIgnoreStationsData::InitFromSettings(const std::string &theBaseNameSpace)
{
	...
	fUseListWithContourDraw = NFmiSettings::Require<bool>(std::string(itsBaseNameSpace + "::UseListWithContourDraw"));
	fUseListWithSymbolDraw = NFmiSettings::Require<bool>(std::string(itsBaseNameSpace + "::UseListWithSymbolDraw"));
	std::string stationListStr = NFmiSettings::Require<std::string>(std::string(itsBaseNameSpace + "::IgnoreStationList"));
...
void NFmiIgnoreStationsData::StoreToSettings(void)
{
	...
	{
		NFmiSettings::Set(std::string(itsBaseNameSpace + "::UseListWithContourDraw"), NFmiStringTools::Convert(fUseListWithContourDraw), true);
		NFmiSettings::Set(std::string(itsBaseNameSpace + "::UseListWithSymbolDraw"), NFmiStringTools::Convert(fUseListWithSymbolDraw), true);
		NFmiSettings::Set(std::string(itsBaseNameSpace + "::IgnoreStationList"), MakeStationListString(), true);
	}
	else
		throw std::runtime_error("Error in NFmiIgnoreStationsData::StoreToSettings, unable to store setting.");
}
...
std::string NFmiIgnoreStationsData::MakeStationListString(void)
{
	...
	{
		int counter = 0;
		for(std::list<NFmiIgnoreStation>::iterator it = itsStationList.begin(); it != itsStationList.end(); ++it)
		{
			if(counter != 0)
