...
{
    // cachejen alustuksia tehdään vain asemadatoille. Hila datat hanskataan emoluokassa ja sitä en lähde tässä vielä optinmoimaan.
...
{
    if(theInfo->IsGrid())
        return std::vector<unsigned long>(); // Hiladatoista käydään kaikki pisteet toistaiseksi (laskut tehdään emoluokassa ja sinne ei saa tätä optimointia)
    ...
    else
    {
        ...
        if(locationIndexCache.empty())
            locationIndexCache.push_back(gMissingIndex); // Jos ei löytynyt laskettavan alueen sisältä yhtään paikkaa, lisätään yksi puuttuva indeksi vektoriin, muuten data käydään läpi kokonaisuudessaa.
