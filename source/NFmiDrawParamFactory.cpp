                          ...
                          bool createDrawParamFileIfNotExist)
{
  if (theDrawParam)
  {
    ...
    {
      // laitetaan tiedoston nimi kuitenkin talteen drawparamiin ja yritet‰‰n luoda tiedosto jos
      // tarpeen
      ...
      if (createDrawParamFileIfNotExist)
      {
        ...
        {  // jos tosiaan tiedostoa ei ollut, luodaan sellainen.
           // Mutta jos luku ei onnistunut, ei luoda sit‰, koska kyseess‰ saattoi olla jokin
           // hetkellinen luku h‰iriˆ
           // ja omituisesti tiuhtissa olevat perustiedostot ovat aina silloin t‰llˆin
           // 'resetoituneet'.
...
// t‰m‰ mahdollisesti tuhoaa annetun DrawParamin tai sitten palautta sen jatkok‰sittelyj‰ varten.
// Joten
// HUOM!!!! Eli parametrina annettua theDrawParam:ia pit‰‰ tarkastaa t‰m‰n kutsun j‰lkeen, koska se
// on
// saatettu tuhota!!!!!
// Rumaa koodia, oikea tapa olisi ehk‰ tehd‰ nuo kolme metodia (CreateDrawParam,
    ...
    else
    {
      ...
      {  // hybrideill‰ on ain yksi piirtotapa
        fileName += "_hybrid";
               ...
               drawParam->Level().LevelType() == kFmiSoundingLevel)  // t‰m‰ on luotaus dataa
