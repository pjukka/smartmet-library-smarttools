...
void NFmiStation2GridMask::DoGriddingCheck(const NFmiCalculationParams &theCalculationParams)
{
	...
	{ 
		WriteLock lock(*itsCacheMutex); // t‰st‰ saa edet‰ vain yksi s‰ie kerrallaan, eli joku joka ehtii, laskee cache-matriisin ja asettaa sen itsCurrentGriddedStationData:n arvoksi, muut sitten vain k‰ytt‰v‰t sit‰
...
NFmiNearestObsValue2GridMask::NFmiNearestObsValue2GridMask(Type theMaskType, NFmiInfoData::Type theDataType, boost::shared_ptr<NFmiFastQueryInfo> &theInfo,
...
NFmiNearestObsValue2GridMask::NFmiNearestObsValue2GridMask(const NFmiNearestObsValue2GridMask &theOther)
:NFmiInfoAreaMask(theOther)
...
double NFmiNearestObsValue2GridMask::Value(const NFmiCalculationParams &theCalculationParams, bool /* fUseTimeInterpolationAlways */ )
{
	DoNearestValueGriddingCheck(theCalculationParams);
	if(itsCurrentNearestObsValuesData)
		return itsCurrentNearestObsValuesData->GetValue(theCalculationParams.itsLocationIndex, kFloatMissing);
	else
		return kFloatMissing;
}

void NFmiNearestObsValue2GridMask::SetGriddingHelpers(NFmiArea *theArea, NFmiEditMapGeneralDataDoc *theDoc, const NFmiPoint &theResultGridSize)
{
	itsAreaPtr.reset(theArea->Clone());
	itsDoc = theDoc;
...
void NFmiNearestObsValue2GridMask::SetArguments(std::vector<float> & theArgumentVector)
{
	...
	if(static_cast<int>(itsArgumentVector.size()) != itsFunctionArgumentCount - 1) // -1 tarkoittaa ett‰ funktion 1. argumentti tulee suoraan itsIfo:sta, eli sit‰ ei anneta argumentti-listassa
		throw std::runtime_error("Internal SmartMet error: Probability function was given invalid number of arguments, cannot calculate the macro.");
}

static NFmiDataMatrix<float> CalcNearestValueMatrix(const NFmiLevel &theLevel, const NFmiCalculationParams &theCalculationParams, const NFmiPoint &theResultGridSize, checkedVector<boost::shared_ptr<NFmiFastQueryInfo> > &theInfoVector, boost::shared_ptr<NFmiArea> &theAreaPtr, float theTimePeekInHours)
{
    // Luodaan tulos matriisi t‰ytettyn‰ puuttuvilla arvoilla
    NFmiDataMatrix<float> nearestValueMatrix(static_cast<NFmiDataMatrix<float>::size_type>(theResultGridSize.X()), static_cast<NFmiDataMatrix<float>::size_type>(theResultGridSize.Y()), kFloatMissing);
    if(theAreaPtr && theInfoVector.size())
    {
        const double kMissingDistanceValue = 9999999999.; // et‰isyys matriisi alustetaan t‰ll‰ suurella luvulla, jolloin aina riitt‰‰ yksi < -vertailu, kun etsit‰‰n l‰hint‰ asemaa
        NFmiDataMatrix<double> distanceMatrix(nearestValueMatrix.NX(), nearestValueMatrix.NY(), kMissingDistanceValue);
        NFmiGrid grid(theAreaPtr.get(), static_cast<unsigned long>(nearestValueMatrix.NX()), static_cast<unsigned long>(nearestValueMatrix.NY()));
        for(auto infoIter : theInfoVector)
        {
            // data ei saa olla hiladataa, eik‰ ns. laivadataa (lokaatio muuttuu ajan myˆt‰ ja lat/lon arvot ovat erillisi‰ parametreja)
                        ...
                        if(theAreaPtr->IsInside(latlon)) // aseman pit‰‰ olla kartta-alueen sis‰ll‰
                        {
                            ...
                            if(grid.NearestLatLon(latlon.X(), latlon.Y()))
                            {
                                ...
                                if(distance < distanceMatrix[grid.CurrentX()][grid.CurrentY()])
                                {
                                    ...
                                    NFmiStationView::SetSoundingDataLevel(theLevel, *infoIter); // T‰m‰ tehd‰‰n vain luotaus datalle: t‰m‰ level pit‰‰ asettaa joka pisteelle erikseen, koska vakio painepinnat eiv‰t ole kaikille luotaus parametreille samoilla leveleill‰
                                    nearestValueMatrix[grid.CurrentX()][grid.CurrentY()] = infoIter->FloatValue(); // ei ole v‰li‰ onko l‰himm‰n aseman arvo puuttuva vai ei, se halutaan aina tulokseen!!!
                                }
                            }
                        }
                    }
...
void NFmiNearestObsValue2GridMask::DoNearestValueGriddingCheck(const NFmiCalculationParams &theCalculationParams)
{
	if(itsLastCalculatedTime != theCalculationParams.itsTime)
	{ 
		WriteLock lock(*itsCacheMutex); // t‰st‰ saa edet‰ vain yksi s‰ie kerrallaan, eli joku joka ehtii, laskee cache-matriisin ja asettaa sen itsCurrentGriddedStationData:n arvoksi, muut sitten vain k‰ytt‰v‰t sit‰
		...
		else
		{
			...
			if(itsDoc && itsAreaPtr.get())
			{
                ...
                float timePeekInHours = itsArgumentVector[0]; // kuinka paljon kurkataan ajassa eteen/taakse kun arvoa haetaan t‰h‰n ajanhetkeen

				boost::shared_ptr<NFmiDrawParam> drawParam(new NFmiDrawParam(itsDataIdent, itsLevel, 0, itsDataType));
                checkedVector<boost::shared_ptr<NFmiFastQueryInfo> > infoVector; // t‰h‰n haetaan tarvittavat datat (synopin tapauksessa mahdollisesti lista)
                itsDoc->MakeDrawedInfoVectorForMapView(infoVector, drawParam, itsAreaPtr);
                NFmiDataMatrix<float> nearestValueData = ::CalcNearestValueMatrix(itsLevel, theCalculationParams, itsResultGridSize, infoVector, itsAreaPtr, timePeekInHours);
                std::pair<DataCache::iterator, bool> insertResult = itsNearestObsValuesData->insert(std::make_pair(theCalculationParams.itsTime, nearestValueData));
				if(insertResult.second)
