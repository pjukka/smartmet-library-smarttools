...
void NFmiNearestObsValue2GridMask::SetArguments(std::vector<float> & theArgumentVector)
{
	...
	if(static_cast<int>(itsArgumentVector.size()) != itsFunctionArgumentCount - 1) // -1 tarkoittaa että funktion 1. argumentti tulee suoraan itsIfo:sta, eli sitä ei anneta argumentti-listassa
...
static NFmiDataMatrix<float> CalcNearestValueMatrix(const NFmiLevel &theLevel, const NFmiCalculationParams &theCalculationParams, const NFmiPoint &theResultGridSize, checkedVector<boost::shared_ptr<NFmiFastQueryInfo> > &theInfoVector, boost::shared_ptr<NFmiArea> &theAreaPtr, float theTimePeekInHours)
{
    ...
    {
        const double kMissingDistanceValue = 9999999999.; // etäisyys matriisi alustetaan tällä suurella luvulla, jolloin aina riittää yksi < -vertailu, kun etsitään lähintä asemaa
        ...
        {
            // data ei saa olla hiladataa, eikä ns. laivadataa (lokaatio muuttuu ajan myötä ja lat/lon arvot ovat erillisiä parametreja)
                        ...
                        if(theAreaPtr->IsInside(latlon)) // aseman pitää olla kartta-alueen sisällä
                        {
                            ...
                            if(grid.NearestLatLon(latlon.X(), latlon.Y()))
                            {
                                ...
                                if(distance < distanceMatrix[grid.CurrentX()][grid.CurrentY()])
                                {
                                    ...
                                    NFmiStationView::SetSoundingDataLevel(theLevel, *infoIter); // Tämä tehdään vain luotaus datalle: tämä level pitää asettaa joka pisteelle erikseen, koska vakio painepinnat eivät ole kaikille luotaus parametreille samoilla leveleillä
                                    nearestValueMatrix[grid.CurrentX()][grid.CurrentY()] = infoIter->FloatValue(); // ei ole väliä onko lähimmän aseman arvo puuttuva vai ei, se halutaan aina tulokseen!!!
                                }
                            }
                        }
                    }
	...
	{ 
		...
		else
		{
			...
			if(itsDoc && itsAreaPtr.get())
			{
                ...
                float timePeekInHours = itsArgumentVector[0]; // kuinka paljon kurkataan ajassa eteen/taakse kun arvoa haetaan tähän ajanhetkeen

				boost::shared_ptr<NFmiDrawParam> drawParam(new NFmiDrawParam(itsDataIdent, itsLevel, 0, itsDataType));
                checkedVector<boost::shared_ptr<NFmiFastQueryInfo> > infoVector; // tähän haetaan tarvittavat datat (synopin tapauksessa mahdollisesti lista)
