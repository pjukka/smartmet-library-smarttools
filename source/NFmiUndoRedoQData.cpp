...
bool NFmiUndoRedoQData::SnapShotData(const std::string& theAction, const NFmiHarmonizerBookKeepingData &theHarmonizerBookKeepingData, const NFmiRawData &theRawData)
{
...
bool NFmiUndoRedoQData::UndoData(const NFmiHarmonizerBookKeepingData &theHarmonizerBookKeepingData, NFmiRawData &theRawData)
{
	if(itsCurrentUndoLevelPtr == 0)
		return false;
	if ((*itsCurrentUndoLevelPtr) < 0)
	...
	{
		std::string action("");
		SnapShotData(action, theHarmonizerBookKeepingData, theRawData);		// "Ottaa kuvan" undo-toimintoa edeltäneestä tilanteesta,
...
bool NFmiUndoRedoQData::RedoData(NFmiRawData &theRawData)
{
	if(itsCurrentUndoLevelPtr == 0)
		return false;
	if((*itsCurrentUndoLevelPtr) == (*itsCurrentRedoLevelPtr) || ((*itsCurrentUndoLevelPtr) + 1) == (*itsCurrentRedoLevelPtr))
		return false;
	else
	{
