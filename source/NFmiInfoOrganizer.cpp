...
NFmiSmartInfo* NFmiInfoOrganizer::Info ( const FmiParameterName& theParam
									   ...
									   , NFmiInfoData::Type theType)
{
	if(theParam == 997) // synop plot paramille pitää tehdä kikka
...
NFmiSmartInfo* NFmiInfoOrganizer::Info ( const NFmiDataIdent& theDataIdent
									   ...
									   , NFmiInfoData::Type theType)
{
	if(theDataIdent.GetParamIdent() == 997) // synop plot paramille pitää tehdä kikka
															...
															,NFmiInfoData::Type theType)
{
	...
	if(theUsedInfo && theUsedInfo->DataType() == theType)
	{
		aSubParam = theUsedInfo->UseSubParam();
		NFmiSmartInfo* copyOfInfo = new NFmiSmartInfo(*theUsedInfo);
		drawParam = itsDrawParamFactory->CreateDrawParam(copyOfInfo, theDataIdent, aSubParam, theLevel, false); // false merkitsee, että parametria ei taas aseteta tuolla metodissa
	}
	return drawParam;
}
