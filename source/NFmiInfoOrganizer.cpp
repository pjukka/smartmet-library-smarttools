								 ...
								 ,bool &fDataWasDeletedOut)
{
	...
	if(theData)
	{
		if(theDataType == NFmiInfoData::kEditable)
			...
			status = Add(new NFmiOwnerInfo(theData, theDataType, theDataFileName, theDataFilePattern), theMaxLatestDataCount, theModelRunTimeGap, fDataWasDeletedOut); // muun tyyppiset datat kuin editoitavat menevät mappiin
	}
	return status;
}
									   ...
									   , int theModelRunIndex)
{
	...
	if(theDataIdent.GetParamIdent() == NFmiInfoData::kFmiSpMetarPlot) // metar plot paramille pitää tehdä kikka (9995)
		return GetMetarPlotParamInfo(theType);
    if(theLevel && theLevel->LevelType() == kFmiSoundingLevel && (theDataIdent.GetParamIdent() == NFmiInfoData::kFmiSpSoundingPlot || theDataIdent.GetProducer()->GetIdent() == kFmiTEMP)) // sounding plot paramille pitää tehdä kikka
...
static bool CheckForParams(boost::shared_ptr<NFmiFastQueryInfo> &theInfo, const std::vector<FmiParameterName> &wantedParams, bool trajectorySpecial = false)
{
    if(::CheckForVerticalData(theInfo))
    {
        ...
        if(trajectorySpecial) // trajektori parametreista pitää olla kolme neljästä, muuten laskuja ei tehdä ollenkaan (tämä ei ole täydellinen tarkistus, koska pitää olla WS, WD ja toinen w -parametreista)
        {
...
bool NFmiInfoOrganizer::HasGoodParamsForSoundingData(boost::shared_ptr<NFmiFastQueryInfo> &theInfo, const ParamCheckFlags &paramCheckFlags)
{
    ...
    if(paramCheckFlags.fSounding)
        ...
        return ::CheckForParams(theInfo, itsWantedTrajectoryParams, true); // trajektori datassa pitää olla kaikki vaaditut parametrit, muuten laskuista ei tule mitään
