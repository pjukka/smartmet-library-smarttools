	...
	for(theResultInfo->ResetLocation(); theResultInfo->NextLocation(); )
	{
		try
		{
			...
			for(theResultInfo->ResetParam(); theResultInfo->NextParam(); )
			{
                counter++;
				if(counter%20 == 0)
					::CheckIfStopped(theStopFunctor); // joka 20 hila/paramtrilla -pisteellä katsotaan, pitääkö lopettaa
...
void NFmiSoundingIndexCalculator::CalculateWholeSoundingData(NFmiQueryData &theSourceData, NFmiQueryData &theResultData, bool useFastFill, bool fDoCerrReporting, NFmiStopFunctor *theStopFunctor, bool fUseOnlyOneThread, int theMaxThreadCount)
{
	...
	if(theMaxThreadCount > 0 && usedThreadCount > static_cast<unsigned int>(theMaxThreadCount))
		usedThreadCount = static_cast<unsigned int>(theMaxThreadCount); // jos on haluttu säätää maksim threadien määrää, säädetään maksimia jos usedThreadCount olisi muuten ylittänyt sen.
	...
	else
	{
		...
		theSourceData.LatLonCache(); // Ennen multi-thread laskuja pitää varmistaa että kunkin datan (source + result) latlon-cache on alustettu, muutern tulee ongelmia.
...
boost::shared_ptr<NFmiQueryData> NFmiSoundingIndexCalculator::CreateNewSoundingIndexData(boost::shared_ptr<NFmiQueryData> sourceData, const std::string &theProducerName, bool fDoCerrReporting, NFmiStopFunctor *theStopFunctor, bool fUseOnlyOneThread, int theMaxThreadCount)
{
    ...
    if(theStopFunctor && theStopFunctor->Stop())
        return boost::shared_ptr<NFmiQueryData>(); // Jos Esim. SmartMetia ollaan sulkemassa ja laskut on lopetettu kesken, ei haluta tallettaa kesken jääneitä laskuja tiedostoon, eli palautetaan tässä tyhjä smart-pointer
    else
    	return data;
}
