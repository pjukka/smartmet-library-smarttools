...
void NFmiTEMPCode::ForceWantedPressureLevels(NFmiVPlaceDescriptor &theLevels)
{
	...
	std::map<double, TEMPLevelData>::reverse_iterator last = it; // tähän sijoitetaan edellisen levelin data
	...
	do
	{
		...
		{ // molemmat kerrokset ovat jo ylempänä kuin etsitty main level, etsitään sellainen main level, mikä ei ole molempien yläpuolella
			do
			{
		...
		if(p1 >= currentMainLevel && p2 <= currentMainLevel)
		{ // nyt etsitty main level on leveleiden välissä. katsotaan kumpi on lähempänä ja muutetaan se tarvittaessa halutuksi leveliksi
		...
		{
			T_Dircounter = 0;
			InsertLevelData(resultLevels, (*last).second); // jos lämpötilan suunta vaihtuu, laitetaan leveli talteen
		}
		...
		if(z1 != z2)
		{
			...
			{
				InsertLevelData(resultLevels, (*last).second); // jos lämpötilan 'kulmakerroin' muuttuu tarpeeksi, laitetaan leveli talteen
...
static bool GetWDandWS(const std::string &thedddff_Str, double &WD, double &WS, bool fConvertKt2Ms)
{
	...
	// HUOM! jos tuulen suunta tai nopeus puuttuu, niitä voisi hanskata paremmin ja laskea toisen vaikka
	// toinen puuttuisi, nyt molemmat puuttuviksi jos toinen puuttuu (tuuliviiriä ei saa kuitenkaan piirretyksi)
...
static bool DecodeHeader(std::stringstream &ssin, NFmiStation &theStation, NFmiMetTime &theTime, bool &fTempMobil, char theWantedSectionChar, bool fInitializeValues, NFmiAviationStationInfoSystem *theTempStations, const NFmiPoint &theUnknownStationLocation, bool &fConvertKt2Ms)
{
    ...
    if(first_Str.size() != 6 && std::isdigit(first_Str[0]))
    	ssin >> first_Str; // joskus 1. stringinä on pitkä luku kutem esim. 0000034101 tai lyhyt luku kuten 999, tämä pitää ohittaa
	...
	if(::toupper(MiMiMjMj_Str[0]) == 'I' && ::toupper(MiMiMjMj_Str[1]) == 'I') // jos stringi muotoa II?? on kyseessä TEMP MOBIL
		doTempMobil = true;
	else if(::toupper(MiMiMjMj_Str[0]) == 'U' && ::toupper(MiMiMjMj_Str[1]) == 'U') // jos stringi muotoa UU?? on kyseessä SHIP, joka on samanlainen kuin TEMP MOBIL
	{
		doTempMobil = true;
		doShip = true;
	}
	...
	if(::toupper(MiMiMjMj_Str[2]) == theWantedSectionChar && ::toupper(MiMiMjMj_Str[3]) == theWantedSectionChar) // stringin pitää olla muotoa ??AA, ??BB, ??CC tai ??DD
	{
		...
		{
			fConvertKt2Ms = true;
			day -= 50; // jos havaintopäivä > 50, on tuulen nopeus solmuissa (vähennä myös se 50 että saat päivän)
		}
		NFmiMetTime tempTime;
		if(day > tempTime.GetDay()) // pitää päätellä ollaanko sittenkin edellisessä kuukaudessa (jos saatu päivä suurempi kuin nyky päivä)
		{
		...
		if(tempTime.GetDay() > daysInMonth) // jos ongelmia päivien kanssa siirrytttäessä kuukausissa (esim. 31 ja kuussa ei ole kuin 30 päivää), laitetaan paiväksi suurin sallittu päivä siinä kuussa
...
bool NFmiTEMPCode::DecodeA(void)
{
	...
	if(::DecodeHeader(ssin, itsStation, itsTime, fTempMobil, 'A', true, itsTempStations, itsUnknownStationLocation, fConvertKt2Ms))
	{
		if(ssin.good())
		{
			...
			if(dddff_Str.size() != 5)
				return false;
			if(std::string(_99PPP_Str.begin(), _99PPP_Str.begin()+2) == "99") // tarkistetaan että 99-taikasana löytyy
			{
	...
	if(::DecodeHeader(ssin, itsStation, itsTime, fTempMobil, 'B', false, itsTempStations, itsUnknownStationLocation, fConvertKt2Ms))
	{
		if(ssin.good())
		{
			...
			do // Käydään ensin osio, missä on on lämpötilojen käännepisteet
			{
				...
				if(XXPPP_Str == "41414")
				{
					_41414_groupMissing = true; // en ole varma miten tämä pitää handlata kun 21212-ryhmä puuttuu
					break; // nyt tultiin tämän osion loppuun
				}

				std::string PPP_partStr(XXPPP_Str.begin()+2, XXPPP_Str.begin()+5);
				if(PPP_partStr != "///") // joskus tämä voi olla puuttuvaa, skipataan vain conversio, paine jää puuttuvaksi ja kaikki tämän levelin arvot jäävät taltioimatta
...
bool NFmiTEMPCode::DecodeC(void)
{
	...
	{
		if(ssin.good())
			return true; // tarkistetaan vain että on C-osio ja sama luotaus kuin aiemmin, ei pureta vielä enempää
...
bool NFmiTEMPCode::DecodeD(void)
{
	...
	{
		if(ssin.good())
			return true; // tarkistetaan vain että on D-osio ja sama luotaus kuin aiemmin, ei pureta vielä enempää
...
NFmiQueryData* DecodeTEMP::MakeNewDataFromTEMPStr(const std::string &theTEMPStr, std::string &theCheckReportStr, NFmiAviationStationInfoSystem &theTempStations, const NFmiPoint &theUnknownStationLocation, const NFmiProducer &theWantedProducer, bool fRoundTimesToNearestSynopticTimes)
{
	...
	{ // kokeillaan sitten jakaa TT -stringin avulla koodia osiin. =-merkkiä ei ole wyomingista saatavista raaka luotaus koodeissa
	...
	for(int i=0; i<static_cast<int>(codeParcels.size()); )
	{
		...
		if(decodeCount > 0) // jos edes yksi osa on onnistuttu purkamaan (4:stä mahdollisesta), laitetaan luotaus listaan
		{
		...
		if(decodeCount <= 0)
			decodeCount = 1; // jos ei onnistunut purkamaan mitään, mennään ainakin yksi blokki eteenpäin ja kokeillaan
