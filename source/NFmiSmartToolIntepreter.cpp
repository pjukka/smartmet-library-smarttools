	  ...
	  if(numericPart.IsNumeric())
		{
		  ...
		  FmiLevelType levelType = GetLevelType(theDataType, levelValue);
			if(levelType == kFmiPressureLevel)
				levelType = kFmiHybridLevel; // jos k‰ytt‰j‰ on antanut lev45, t‰llˆin halutaan hybrid level 45 ei painepinta 45. painepinnat saa automaattisesti pelk‰ll‰ numerolla
