...
void NFmiModifiableQDatasBookKeeping::CopyClonedDatas(const NFmiModifiableQDatasBookKeeping &theOther)
{
	if(this != &theOther)
	{
...
int NFmiModifiableQDatasBookKeeping::MaskedCount(unsigned long theMaskType, unsigned long theIndex, const NFmiRect& theSearchArea, unsigned long theGridXNumber, unsigned long theGridYNumber)
{
	return (*itsAreaMask)->MaskedCount(theMaskType, theIndex, theSearchArea, theGridXNumber, theGridYNumber);
}
...
void NFmiModifiableQDatasBookKeeping::MaskAllLocations(const bool& newState, unsigned long theMaskType)
{
...
void NFmiModifiableQDatasBookKeeping::MaskLocation(const bool& newState, unsigned long theMaskType, unsigned long theLocationIndex)
{
...
bool NFmiModifiableQDatasBookKeeping::SnapShotData(const std::string& theAction, const NFmiHarmonizerBookKeepingData &theHarmonizerBookKeepingData, const NFmiRawData &theRawData)
{
	if(itsUndoRedoQData)
...
bool NFmiModifiableQDatasBookKeeping::UndoData(const NFmiHarmonizerBookKeepingData &theHarmonizerBookKeepingData, NFmiRawData &theRawData)
{
	if(itsUndoRedoQData)
	{
