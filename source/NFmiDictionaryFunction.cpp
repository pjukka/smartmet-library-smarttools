...
std::string GetDictionaryString(const char *theMagicWord)
{
    ...
    NFmiStringTools::ReplaceAll(usedMagicWord, "\t", "\\t"); // pit‰‰ muuttaa mahdollinen tabulaattori merkkisarjaksi, miss‰ on kenoviiva ja t per‰kk‰in, koska tabulaattori on siten sanakirjassa
    ...
    NFmiStringTools::ReplaceAll(dictionarySentense, "\\t", "\t"); // pit‰‰ muuttaa mahdolliset "\\t"-merkkijonot takaisin tabulaattoreiksi
