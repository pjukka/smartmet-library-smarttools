...
bool NFmiSoundingData::GetValuesNeededInLCLCalculations(FmiLCLCalcType theLCLCalcType, double &T, double &Td, double &P)
{
	...
	{
		P=1100;
		if(!GetValuesStartingLookingFromPressureLevel(T, Td, P)) // sitten lähimmät pinta arvot, jotka joskus yli 500 m
			return false;
	}
	return true;
}
	...
	for(size_t i = 0; i < ssize; i++)
	{
		if(pValues[i] != kFloatMissing && pValues[i] <= P) // aloitetaan LFC etsintä vasta 'aloitus' korkeuden jälkeen
		{
			if(tValues[i] != kFloatMissing) // kaikilla painepinnoilla ei ole lämpötilaa
			{
				...
				if(durrentDiff < 0 && lastDiff > 0) // jos siis paketti muuttui lämpimämmästä kylmemmäksi (ympäristöön verrattuna, ota talteen korkeus)
					EL = pValues[i];
		...
		if(pValues[i] != kFloatMissing && pValues[i] < P) // aloitetaan LFC etsintä vasta 'aloitus' korkeuden jälkeen
		{
			if(tValues[i] != kFloatMissing) // kaikilla painepinnoilla ei ole lämpötilaa
			{
				...
				if(theHeightLimit != kFloatMissing && currentZ > theHeightLimit)
					break; // lopetetaan laskut jos ollaan menty korkeus rajan yli, kun ensin on laskettu tämä siivu vielä mukaan
			}
		}
	}
	return CAPE;
...
// startH and endH are in kilometers
// käytetään muuttujan niminä samoja mitä on Pieterin helpissä, vaikka kyseessä ei olekaan laskut layerille 0-6km vaan mille välille tahansa
// HUOM! Pitää ottaa huomioon aseman korkeus kun tehdään laskuja!!!!
