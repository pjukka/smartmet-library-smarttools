...
//	value = FmiMakeValidNumber(value); // NFmiQueryInfo::FloatValue(value) -metodi tarkastaa nykyään onko kyseessä sallittu luku, inf ja nan -arvojen asetus on nykyään estetty
	if(value != kFloatMissing) // tuli ongelmia missing asetuksissa, pitää mietti vaikka jokin funktio, jolla asetetaan puuttuva arvo // pitää pystyä sittenkin asettamaan arvoksi kFloatMissing:in!!!
	{
		itsResultInfo->LocationIndex(theCalculationParams.itsLocationIndex); // kohde dataa juoksutetaan, joten lokaatio indeksien pitää olla synkassa!!!
...
{
	if(itsResultInfo) // maskin tapauksessa tämä on 0-pointteri (ja tulevaisuudessa tämä sijoitus info poistuu kokonaisuudessaan)
...
void NFmiSmartToolCalculation::eval_exp6(double &result, const NFmiCalculationParams &theCalculationParams)
{
	...
	else if(token->GetCalculationOperationType() == NFmiAreaMask::MathFunctionStart)
	{
		...
		get_token();
		eval_exp2(result, theCalculationParams);
		if(token->GetCalculationOperationType() != NFmiAreaMask::EndParenthesis) // MathFunctionStart:in päättää normaali lopetus sulku!
...
void NFmiSmartToolCalculation::eval_ThreeArgumentFunction(double &result, double argument1, double argument2, NFmiAreaMask::FunctionType func, int theIntegrationFunctionType, const NFmiCalculationParams &theCalculationParams)
{
	...
	else
	{
		if(argument1 != kFloatMissing && argument2 != kFloatMissing)
		{
			...
			// 1. ota talteen token tai se iteraattori, että samoja laskuja voidaan käydä läpi uudestaan ja uudestaan
			CalcIter startCalcIterator = itsCalcIterator; // pitääkö olla edellinen??, pitää olla epäselvää, mutta pakko
...
void NFmiSmartToolCalculation::eval_ThreeArgumentFunctionZ(double &result, double argument1, double argument2, NFmiAreaMask::FunctionType func, int theIntegrationFunctionType, const NFmiCalculationParams &theCalculationParams)
{
	...
	{
		double value = kFloatMissing;
		double heightValue = kFloatMissing; // tähän talletetaan h-funktion tapauksessa minimin/maksimin korkeus
		// 1. ota talteen token tai se iteraattori, että samoja laskuja voidaan käydä läpi uudestaan ja uudestaan
		CalcIter startCalcIterator = itsCalcIterator; // pitääkö olla edellinen??, pitää olla epäselvää, mutta pakko
		// 3. jos korkeus lasku
		itsHeightValue = static_cast<float>(argument1);
		// 4. mieti mikä on aikahyppy (oletus 1h) jos pelkkää EC-dataa, voisi aikahyppy olla 3h tai 6h
		// HUOM!! Muuta resoluutio siten, että 50, kun korkeus alle 500 m, 100, kun korkeus alle 3000 m, 200 kun korkeus alle 10000 m ja 500 kun korkeus yli sen
...
{
	// huom! tässä ei käytetä bin_eval-kutsuja, koska tässä lasketaan vain yksi luku, mikä palautetaan bin_eval-systeemiin
	NFmiAreaMask::FunctionType func = token->GetFunctionType(); // eli onko kyse min, max vai mistä funktiosta
	int integrationFunctionType = token->IntegrationFunctionType(); // tämä kertoo onko kyse vertikaali vai ajallisesta integroinnista kyse
	get_token();
	double argument1 = kFloatMissing;
	eval_exp2(argument1, theCalculationParams);
	if(token->GetCalculationOperationType() != NFmiAreaMask::CommaOperator) // näiden funktioiden argumentit erotetaan pilkuilla
...
{
	boost::shared_ptr<NFmiAreaMask> verticalFunctionToken = token; // tähän otetaan talteen vertikaalilasku olio
	int trueArgumentCount = verticalFunctionToken->FunctionArgumentCount() - 1;
	std::vector<float> argumentVector; // tässä vektorissa annetaan vertikaali laskuissa tarvittavat argumentti arvot (Paitsi 1. joka tulee AreaMask-olion omasta itsInfo-datasta)
	for(int argumentCounter = 0; argumentCounter < trueArgumentCount; argumentCounter++)
	{
		...
		if(!(commaOperator || endParethesisOperator)) // näiden funktioiden argumentit erotetaan pilkuilla tai ne loppuu sulkuun
			throw  runtime_error(::GetDictionaryString("SmartToolCalculationErrorInvalidParamSeparation") + ":\n" + GetCalculationText());
...
checkedVector<boost::shared_ptr<NFmiSmartToolCalculation> > NFmiSmartToolCalculation::DoShallowCopy(const checkedVector<boost::shared_ptr<NFmiSmartToolCalculation> > &theCalculationVector)
{
	checkedVector<boost::shared_ptr<NFmiSmartToolCalculation> > returnVector(theCalculationVector.size());
	for(size_t i = 0; i < theCalculationVector.size(); i++)
		returnVector[i] = boost::shared_ptr<NFmiSmartToolCalculation>(new NFmiSmartToolCalculation(*theCalculationVector[i]));
	return returnVector;
}
