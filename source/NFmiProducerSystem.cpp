...
void NFmiProducerSystem::InitFromSettings(const std::string &theInitNameSpace)
{
    ...
    const int maxConsecutiveMissingCount = 20; // ei kuitenkaan k‰yd‰ l‰pi kaikkia 1000 mahdollisuutta, vaan jos on ollut n‰in monta puuttuvaa per‰kk‰in, niin lopetetaan
	for(int i=1; i <= maxDirectoryCount; i++)
	{
        ...
        if(consecutiveMissingCount > maxConsecutiveMissingCount)
            break; // tein t‰m‰n skippauksen, koska debug versiossa 1000 l‰pik‰ynti on aika hidasta, release versiossa ei juuri huomaa
	}
	if(prodCounter > 0)
	{
...
// Producer-matodilla. Palautuva indeksi on siis 1:st‰ l‰htev‰ ja 0 tarkoittaa ett‰ kyseit‰ tuottajaa ei lˆytynyt.
