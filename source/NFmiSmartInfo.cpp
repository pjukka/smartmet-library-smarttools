...
,itsDataType(theType) // 1999.08.24/Marko
{
	...
	itsAreaFactors = 0;
	fEditable = kTrue; //mik‰ alkuarvoksi?!! 
	itsPriority = 0;
...
//   mit‰‰n Undo:ta teht‰viss‰, palautetaan kFalse.
...
FmiBoolean NFmiSmartInfo::UndoData (void)
{
	...
	{
		NFmiString action("");
		SnapShotData(action);		// "Ottaa kuvan" undo-toimintoa edelt‰neest‰ tilanteesta,
	...
	*fDirty = kTrue; // 18.1.2002/Marko lis‰sin datan likauksen.
	return kTrue;
}
...
void NFmiSmartInfo::LocationMaskStepGrid(FmiBoolean newStatus, FmiBoolean fResetAllFirst, const long& theXStep, const long& theYStep)
{
	...
	unsigned long midX = (unsigned long)(theXStep/2); // sijoittaa n‰kyv‰t hilat 'keskemm‰lle'
