...
bool NFmiSoundingDataOpt1::FastFillParamData(const boost::shared_ptr<NFmiFastQueryInfo> &theInfo, FmiParameterName theId)
{
	...
	else if(theId == kFmiDewPoint && theInfo->Param(kFmiHumidity))
	{
		...
		for(theInfo->ResetLevel(); theInfo->NextLevel(); i++)
		{
			...
			RH = theInfo->FloatValue(); // varmuuden vuoksi kaikki interpoloinnit päälle, se funktio tarkistaa tarvitseeko sitä tehdä
			...
			if(theGroundDataInfo && itsPressureData.size() > 0)
			{
				...
				for(int i=0; i < static_cast<int>(itsPressureData.size()); i++)
				{
					...
					{ // Eli nyt luotausdata taulukosta löytynyt paine on pienempi kuin paineasemakorkeudella (eli ollaan juuri menty
						...
						{ 
							// tämä voi käydä esim. hybridi datan kanssa, eli hybridi luotaukseen lisätään alkuun pinta arvot
							itsZeroHeightIndex = 0;
							itsZeroHeight = 2;
...
double NFmiSoundingDataOpt1::CalcLFCIndex(FmiLCLCalcType theLCLCalcType, double &EL)
{
	...
	{
		// 4. Käy läpi kaikki ei puuttuvat P ja T kerrokset. Lisäksi aloitus korkeuden pitää olla sama (tai korkeammalla oleva pinta) 
		// kuin saatiin P:n arvoksi aiemmin.
		P_current = pValues[i];
		T_current = tValues[i];
		if(P_current != kFloatMissing && T_current != kFloatMissing && P_current <= P)
		{
				// 5. LCL-pisteeseen pitää saada yksi laskentapiste, muuten laskut menevät tietyissä tilanteissa vähän pieleen
				...
				{ // Nyt mentiin CAPE-alueeseen, pitää lisätä kaksi pistettä eli entry-piste ja currentti piste
