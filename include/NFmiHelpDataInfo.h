...
private:
	...
	std::string itsImageProjectionString; // jos data on image-tyyppistä (satel,radar, jne.), on tiedostossa
    ...
    boost::shared_ptr<NFmiArea> itsImageArea; // tähän luodaan ed. stringin avulla projektio, tämä ei ole tallessa tiedostossa
	...
	int itsReportNewDataTimeStepInMinutes; // Default arvo on 0, jolloin tällä ei ole vaikutusta. Tämän avulla voidaan sanoa
...
private:
	...
	float itsCacheFileKeepMaxDays; // kuinka vanhat tiedostot ainakin siivotaan pois (esim. 1.5 on 1.5 päivää eli 36 tuntia) jos luku on <= 0 ei tätä käytetä
	int itsCacheMaxFilesPerPattern; // kuinka monta tiedostoa maksimissaan pidetään kutakin tiedosto patternia kohden, jos luku <= 0, ei tätä käytetä
