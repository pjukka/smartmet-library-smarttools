...
private:
	...
	bool fMacroParamCalculation; // tämä tieto tarvitaan scriptVariablejen kanssa, jos true, käytetään pohjana macroParam-infoa, muuten editoitua dataa
	...
	bool fHeightFunctionFlag; // ollaanko tekemässä SumZ tai MinH tyyppisen funktion calculaatio-otusta
	bool fUseLevelData; // kun tämä flagi on päällä, käytetään CreateInfo-metodissa hybridi-datoja jos mahd. ja sitten painepinta datoja.
	bool fDoCrossSectionCalculation; // kun tämä flagi on päällä, ollaan laskemassa poikkileikkauksia ja silloin level-infot yritetään tehdä ensin hybrididatasta ja sitten painepintadatasta
	int itsCommaCounter; // tarvitaan laskemaan pilkkuja, kun lasketaan milloin level-dataa pitää käyttää.
