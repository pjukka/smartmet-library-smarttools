...
class NFmiDrawParam
{
 public:
  ...
  void IsolineLabelBoxFillColor(const NFmiColor& theColor)
  {
    ...
    itsContourLabelBoxFillColor =
        theColor;  // **** Versio 3 parametri asetetaan toistaiseksi myˆs n‰in ****
  };
  ...
  void UseIsoLineGabWithCustomContours(const bool newState)
  {
    ...
    fUseContourGabWithCustomContours =
        newState;  // **** Versio 3 parametri asetetaan toistaiseksi myˆs n‰in ****
  };
  ...
  void IsolineTextColor(const NFmiColor& newColor)
  {
    ...
    itsContourTextColor =
        newColor;  // **** Versio 3 parametri asetetaan toistaiseksi myˆs n‰in ****
  }
  ...
  void UseIsoLineFeathering(bool newValue)
  {
    ...
    fUseContourFeathering =
        newValue;  // **** Versio 3 parametri asetetaan toistaiseksi myˆs n‰in ****
  }
 ...
 protected:
  ...
  std::string itsMacroParamRelativePath;  // t‰t‰ tarvitaan kun viewMacrojen yhteydess‰ on
                                          // macroParametreja ja
  ...
  NFmiMetEditorTypes::View itsStationDataViewType;  // jos viewtype on isoviiva, mutta data on asema
                                                    // dataa, pit‰‰ olla varalla joku n‰yttˆtyyppi
                                                    // ett‰ voidaan piirt‰‰ t‰llˆin
  ...
  bool fUseSymbolsInTextMode;  // k‰ytet‰‰nkˆ teksti‰ vai mapataanko arvoja kohden jokin symboli
                               // ruudulle?
  ...
  bool fShowColorLegend;  // piirret‰‰nko rivin viimeiseen ruutuun color contouringin v‰ri selitys
                          // laatikko?
  bool fUseSimpleIsoLineDefinitions;  // otetaanko isoviivam‰‰ritykset simppelill‰ tavalla, vai
                                      // kaikki arvot spesiaali tapauksina m‰‰ritettyin‰
  ...
  bool fShowSimpleIsoLineLabelBox;          // ei viel‰ muita attribuutteja isoviiva labelille (t‰m‰
                                            // tarkoittaa lukua ymp‰roiv‰‰ laatikkoa)
  ...
  bool fUseSingleColorsWithSimpleIsoLines;  // true=sama v‰ri kaikille isoviivoille, false=tehd‰‰n
                                            // v‰riskaala
  ...
  checkedVector<float>
      itsSpecialIsoLineValues;  // t‰h‰n laitetaan kaikki arvot, johon halutaan isoviiva
  ...
  checkedVector<int> itsSpecialIsoLineColorIndexies;  // eri viivojen v‰ri indeksit (pit‰‰ tehd‰
                                                      // n‰yttˆ taulukko k‰ytt‰j‰lle)
  checkedVector<int> itsSpecialContourColorIndexies;  // **** Versio 3 parametri ****
  checkedVector<bool> itsSpecialIsoLineShowLabelBox;  // eri viivojen v‰ri indeksit (pit‰‰ tehd‰
                                                      // n‰yttˆ taulukko k‰ytt‰j‰lle)
  ...
  checkedVector<float> itsSpecialColorContouringValues;  // t‰h‰n laitetaan kaikki arvot, johon
                                                         // halutaan color contour luokka rajoiksi
  checkedVector<int> itsSpecialColorContouringColorIndexies;  // eri viivojen v‰ri indeksit (pit‰‰
                                                              // tehd‰ n‰yttˆ taulukko k‰ytt‰j‰lle)
  ...
  int itsContourLabelDigitCount;  // **** Versio 3 parametri ****
                                  ...
                                  //********** 'versio 2' parametreja *************
                                  //***********************************************
  float itsAlpha;  // l‰pin‰kyvyys kerroin, 0 on t‰ysin l‰pin‰kyv‰ ja 100 t‰ysin l‰pin‰kym‰tˆn (0 on
                   // estetty ja itsMinAlpha on s‰‰detty minimi raja)
 ...
 private:
  ...
  // tuulivektori eiv‰t ole)

  std::string itsUnit;
  bool fActive;  // onko kyseinen parametri n‰ytˆn aktiivinen parametri (jokaisella n‰yttˆrivill‰
                 // aina yksi aktivoitunut parametri)
  bool fShowDifference;  // n‰ytet‰‰nkˆ kartalla parametrin arvo, vai erotus edelliseen aikaan (ei
                         // ole viel‰ talletettu tiedostoon)
  bool fShowDifferenceToOriginalData;

  NFmiInfoData::Type itsDataType;  // lis‰sin t‰m‰n, kun laitoin editoriin satelliitti kuvien
  ...
  // Arkistodatan k‰yttˆˆn liittyvi‰ asetuksia (historialliseen dataan voidaan viitata kahdella eri
  // tavalla)
  // Joko suoraan fixatulla itsModelOriginTime:lla tai itsModelRunIndex:ill‰, jolla kerrotaan
  // monesko ajo
  // kiinnostaa viimeiseen n‰hden.
  // Prioriteetti j‰rjestys on, ett‰ ensin tarkistetaan lˆytyykˆ fiksattu aika (itsModelOriginTime)
  // ja sitten vasta onko suhteellinen.
  NFmiMetTime itsModelOriginTime;  // Jos t‰ss‰ arvo, etsit‰‰n arkistosta (SmartMetin
                                   // (tulevaisuudessa) oma tai Q2Server)
  // suoraan kyseinen aika. Jos arvo on NFmiDrawParam::gMissingOriginTime, t‰m‰ ei
  // ole k‰ytˆss‰.
  int itsModelRunIndex;  // Jos t‰ss‰ on negatiivinen arvo (-1 on edellinen malliajo, -2 on sit‰
                         // edellinen jne.), haetaan
  // arkistosta dataa. T‰m‰ luku on siis verrattuna kyseisen mallin viimeisimp‰‰n data osaan, joka
  // lˆytyy.
  ...
  NFmiMetTime itsModelOriginTimeCalculated;  // t‰h‰n lasketaan relatiivisen malliajon mukainen
                                             // origin aika, jot‰ k‰ytet‰‰n sitten mm. tooltipeiss‰
                                             // ja muualla
  ...
  int itsModelRunDifferenceIndex;  // t‰m‰n avulla on tarkoitus verrata eri malliajoja. Jos t‰m‰ on
                                   // >= 0, ei ole vertailua, jos se on -1 verrataan edelliseen
                                   // ajoon, -2:lla verrataan sit‰ edelliseen jne

  // Mallidatoja voidaan verrata analyysi ja havainto datoihin. Siis sellaiseen dataan, mill‰ on
  // vain yhdet arvot kullekin havainto hetkelle.
  unsigned long itsDataComparisonProdId;  // Jos t‰m‰ on 0, ei vertailu-optio ole p‰‰ll‰. Muuten
                                          // t‰ss‰ on halutun vertailudatan tuottaja id
                                          // (analyysi/havainto datan)
  NFmiInfoData::Type
      itsDataComparisonType;  // t‰ss‰ on k‰ytetty datatyyppi esim. kAnalyzeData tai kObservations
};
