...
// Kaikki osiot sikä vapaa ehtoisia, ettei mitään tarvitse olla, mutta esim IF, ELSEIF ja ELSE:n jälkeen pitää tulla calculationSection.
//**********************************************************
...
private:
	...
	bool fContinueCurrentSectionCheckOut; // jatketaanko sen hetkisen sectionin tutkimista, vai onko selvää, ettei esim. else-sectionia ole olemassa
	...
	static checkedVector<std::string> itsTokenMacroParamIdentifiers; // tänne listataan result jne. sanat joita käytetään makrojen visualisoinnissa
	...
	typedef boost::tuple<NFmiAreaMask::FunctionType, NFmiAreaMask::MetFunctionDirection, int, std::string> MetFunctionMapValue; // MetFunktioihin talletetaan 'taika'-sanan lisäksi Funktio tyyppi ja 
	...
	typedef std::map<std::string, MetFunctionMapValue> MetFunctionMap; // 'Meteorologiset' funktiot. Näillä funktioilla käsitellään queryData-olioita eli pyydetään erilaisia 
	...
	typedef boost::tuple<NFmiAreaMask::FunctionType, NFmiAreaMask::FunctionType, int, std::string> VertFunctionMapValue; // Vert(ikaali)Funktioihin talletetaan 'taika'-sanan 
																							// lisäksi Funktio tyyppi1 (avg,max,find, jne.) ja tyyppi2 (vertikaali rajojen muoto, eli ala- ja ylärajojen määritykset,
																							...
																							// joka pitää sisällään esim. grad-funktion tapauksessa "grad(param)"
	typedef std::map<std::string, VertFunctionMapValue> VertFunctionMap; // Vertikaali funktiot. Näillä funktioilla käsitellään queryData-olioita eli pyydetään erilaisia 
																		// arvoja siitä (esim. vertp_max(WS_hir, p1, p2) hakee hirlamin maksimi tuulen nopeuden p1 ja p2 painepintojen väliltä).
