...
private:
	std::string itsName; // Pitempi nimi esim. Hirlam tai Ecmwf (voidaan käyttää esim. popup-valikoissa, missä on tilaa)
	std::vector<std::string> itsShortNameVector; // Lista lyhyemmistä nimistä esim. Hir tai Ec (käytetään nyt ainoastaan smarttool-kielen tuottaja nimenä skripteissä)
	std::string itsUltraShortName; // Lyhyempi nimi esim. Hir tai Ec (käytetään mm. jossain pikavalinnoissa nimenä)
	unsigned long itsProducerId;
	std::string itsDescription;
	bool fHasRealVerticalData; // esim. UK,DWDjaUSa datoissa ei ole nyt oikeasti vertikaalidataa, vaikka siinä on painepintoja, näistä malleista ei haluta
							   // tehdä luotauksia, poikkileikkausia tai trajektori laskuja, joten nämä mallit saavat arvon false.
...
{
public:
	static const unsigned int gHelpEditorDataProdId = 996; // pistin tämän tuottaja id:n tanne, koska en viitsi laittaa tätä
...
private:
	...
	std::vector<NFmiProducerInfo> itsProducers; // Tähän voi siis laittaa tuottajia kuinka paljon tahansa,
	...
	std::string itsInitializeLogStr; // jos luokka initialisoidaan settingeistä, tähän talletetaan loki stringi
};
