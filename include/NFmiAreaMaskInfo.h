...
private:
	...
	std::string itsMaskText; // originaali teksti, mistä tämä maskinfo on tulkittu, tämä on siis vain yksi sana tai luku
	std::string itsOrigLineText; // originaali koko rivin teksti, mistä tämä currentti sana (itsMaskText) on otettu (tätä käytetään virhe teksteissä)
	...
	NFmiAreaMask::FunctionType itsSecondaryFunctionType; // Tähän laitetaan mm. vertikaali funktioissa käytetty korkeus tyyppi esim. VertP tai VertZ
	NFmiAreaMask::MetFunctionDirection itsMetFunctionDirection; // grad, adv, div rot ja lap -funktioille (ja näiden 2-versioille) määrätään myös suunta, joka voi olla X, Y tai molemmat
	...
	int itsFunctionArgumentCount; // kuinka monta pilkulla eroteltua argumenttia on odotettavissa tähän 'meteorologiseen' funktioon (mm. grad, adv, div, lap, rot jne....).
