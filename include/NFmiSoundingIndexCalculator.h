...
public:
	...
	static void CalculateWholeSoundingData(NFmiQueryData &theSourceData, NFmiQueryData &theResultData, bool useFastFill, bool fDoCerrReporting, NFmiStopFunctor *theStopFunctor = 0, bool fUseOnlyOneThread = true, int theMaxThreadCount = 0, bool useFastESATCalculations = false);
	static boost::shared_ptr<NFmiQueryData> CreateNewSoundingIndexData(const std::string &theSourceFileFilter, const std::string &theProducerName, bool fDoCerrReporting, NFmiStopFunctor *theStopFunctor = 0, bool fUseOnlyOneThread = true, int theMaxThreadCount = 0, bool useFastESATCalculations = false);
};
